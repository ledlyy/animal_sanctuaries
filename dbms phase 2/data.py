...
def insert_donations(cursor):
    # this function is inserting donations for 400 users
    donated_users = []
    users_list = get_list(cursor, "users", "user_id")
    try:
        for _ in range(400):
...
def insert_adoptions_application(cursor):
    # this function will insert 300 applications for adpotion_applications table
    users_list = get_list(cursor, "users", "user_id")
...
def insert_care_records(cursor):
    # this function creates data for care records table
...
def insert_employment_history(cursor):
    # this function creates random data for the employment history table
...
def delete_table(cursor, table):
    # this function delets data from a table with given table name
    try:
...
def initialize_all_tables(cursor):
    # this function initializes all tables in the database using the sql file named initialize_all_tables.sql
    try:
...
def insert_all_data(cursor):
    # this function inserts all data into the each table in the database
    try:
...
def main():
    database_name = "animal_sanctuaries"
    ...
    try:
        ...
        # to create new database named "animal_sanctuaries" you can use this function
        #create_database(cursor, database_name)
        # after creating database, you can import meaningful data using faker library also using functions in this file
        # to run function below, you need to change database name with "animal_sanctuaries" in the connection above
        #create_new_random_data(cursor)
        
        
        # or you can import database from sql file which named "animal_sanctuaries_database.sql"
        #use terminal to create database from sql file
        #psql -U postgres -p 5433 -c "CREATE DATABASE animal_sanctuaries;"
        #psql -U postgres -d animal_sanctuaries -f animal_sanctuaries_database.sql -p 5433
